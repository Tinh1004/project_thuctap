...
export default function List({ item, closeHandler, visible }) {
  const [inputName, setInputName] = useState("");
  const dispatch = useDispatch();
  const handleAddPlaylist = () => {
    const id = uuidv4();
    dispatch(
      userSlice.actions.addPlayList({
        name: inputName,
  ...
  return (
    <div>
      <span className="text nav-text">{item ? item.title : ""}</span>
      <div>
        <Modal
        ...
        >
          ...
          <Modal.Body>
            ...
            <Checkbox>
              <div class="a">
                Phát ngẫu nhiên
                <Text size={14}>Luôn phát ngẫu nhiên những bài hát này</Text>
              </div>
            </Checkbox>
